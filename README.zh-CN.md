<div align="center">
  ...
  <p><strong>面向 Codex / Claude Code 的每日语音论文微信公众号自动化 pipeline。</strong></p>
...
Speech Paper WeChat 是一个 agent-first 的端到端 pipeline，用于把每日语音与音频方向 arXiv 论文整理成微信公众号草稿。

本仓库会从 arXiv `cs.SD` 与 `eess.AS` 的 recent 页面中读取指定日期块，下载候选论文 PDF，抽取文本，维护结构化评审记录，生成微信公众号 Markdown，自动生成内容感知封面，并推送到微信公众号草稿箱。

如果这个项目对你有帮助，欢迎点一个 Star。它会帮助项目被更多语音、音频和科研工具开发者看到。
...
<div align="center">
  ...
  <p>这里会发布由本 skill 生成的每日语音/音频论文速递。</p>
</div>

## 🗞️ News

- **2026-05-21**：整理为公开仓库结构，包含每日语音论文公众号完整流程。
- **2026-05-21**：封面生成入口统一为 `nanobanana`，保留旧命令兼容 wrapper。
- **2026-05-20**：已在 arXiv `Wed, 20 May 2026` 的 `cs.SD` 与 `eess.AS` 日期块上验证流程。

## ✨ 功能

- **Agent-first**：面向 Codex、Claude Code 和 OpenClaw 风格 coding agent，核心说明写在 `SKILL.md`。
- **日期块抓取**：只抓取 arXiv recent 页面中的目标日期块。
- **语音/音频范围**：同时覆盖 `cs.SD` 和 `eess.AS`，并按 arXiv ID 合并重复论文。
- **PDF-grounded review**：下载 PDF，并用 `pdftotext` 抽取首页与全文文本。
- **结构化记录**：通过 `reviewed.jsonl` / `reviewed.json` 维护逐篇结构化评审。
- **微信公众号 Markdown**：支持精选版与全量版。
- **封面生成**：通过 `nanobanana` 生成内容感知封面图。
...
| 阶段 | 输出 |
| --- | --- |
| 论文抓取 | `metadata_today.json`、PDF、抽取文本 |
...
`config/nanobanana.env`、`.env`、PDF、图片和运行产物都已被 git 忽略。
...
```text
使用本仓库的 SKILL.md 生成每日语音论文微信公众号草稿。
目标 arXiv 日期标题是："Wed, 20 May 2026"。
只读取 cs.SD 和 eess.AS 中这个日期块的论文。
忽略 replacement entries。
下载 PDF，阅读论文文本，生成 reviewed.jsonl，生成微信公众号 Markdown，
用 nanobanana 生成封面，并在确认所有 authors_org 都完整后推送多图文草稿。
```

只生成草稿、不发布：

```text
运行 speech-paper-wechat pipeline，处理今天 arXiv 的语音/音频论文。
...
```text
抓取 cs.SD 和 eess.AS 的目标 arXiv 日期块，下载 PDF，并抽取首页和全文文本。
不要写评审，不要发布。
```
