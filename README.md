...
`.env`, generated PDFs, images, and bulky run outputs are ignored by git. Final WeChat Markdown drafts and `wechat-post-multi.json` under `runs/` may be committed for review/history.
...
generated cover/full-text extraction outputs. Final WeChat Markdown drafts and
`wechat-post-multi.json` can be committed after review. Keep all production
credentials in local ignored files or environment variables.
