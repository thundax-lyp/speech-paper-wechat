...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--type" && argv[i + 1]) {
      ...
      args.dryRun = true;
    } else if (arg === "--multi-manifest" && argv[i + 1]) {
      args.multiManifest = argv[++i];
...
async function main(): Promise<void> {
  ...
  // Multi-manifest: render additional articles and publish together
  if (args.multiManifest && fs.existsSync(args.multiManifest)) {
    console.error(`[wechat-api] Multi-article manifest detected: ${args.multiManifest}`);
    const manifest = JSON.parse(fs.readFileSync(args.multiManifest, "utf-8"));
    const allArticles: any[] = [];

    for (const entry of manifest.articles) {
      const entryPath = path.resolve(entry.path);
      if (!fs.existsSync(entryPath)) {
        console.error(`[wechat-api] Skipping missing file: ${entryPath}`);
        continue;
      }
      const entryContent = fs.readFileSync(entryPath, "utf-8");
      const entryParsed = parseFrontmatter(entryContent);
      const entryBody = entryParsed.body;
      let entryTitle = entry.title || entryParsed.frontmatter.title || "";
      if (!entryTitle) {
        const h1 = entryBody.match(/^#\s+(.+)$/m);
        if (h1) entryTitle = h1[1]!;
      }
      let entryAuthor = author || entryParsed.frontmatter.author || "";
      let entryDigest = entryParsed.frontmatter.digest || entryParsed.frontmatter.summary || entryParsed.frontmatter.description || "";
      if (entryDigest && entryDigest.length > 120) {
        const t = entryDigest.slice(0, 117);
        const lp = Math.max(t.lastIndexOf("\u3002"), t.lastIndexOf("\uff0c"), t.lastIndexOf("\uff1b"), t.lastIndexOf("\u3001"));
        entryDigest = lp > 80 ? t.slice(0, lp + 1) : t + "...";
      }

      console.error(`[wechat-api] Rendering article: ${entryTitle}`);
      const entryRendered = renderMarkdownWithPlaceholders(entryPath, args.theme, args.color, args.citeStatus, entry.title);
      let entryHtml = extractHtmlContent(entryRendered.htmlPath);
      const entryBaseDir = path.dirname(entryPath);

      const entryUploaded = await uploadImagesInHtml(
        entryHtml, accessToken, entryBaseDir, entryRendered.contentImages,
        args.articleType, false,
      );
      entryHtml = entryUploaded.html;
      ...
      const art: any = {
        ...
        need_open_comment: resolved.need_open_comment ? 1 : 0,
        only_fans_can_comment: resolved.only_fans_can_comment ? 1 : 0,
      };
      if (entryAuthor) art.author = entryAuthor;
      if (entryDigest) art.digest = entryDigest;
      allArticles.push(art);
    }

    console.error(`[wechat-api] Publishing to draft... (${allArticles.length} article(s))`);
    ...
    const titles = allArticles.map((a: any) => a.title);
    console.log(JSON.stringify({
      success: true,
    ...
    console.error(`[wechat-api] Published successfully! media_id: ${data.media_id}`);
  } else {
